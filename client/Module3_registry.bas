...
Function DeleteValue(Keyname As String, Entry As String) ', Value As String)
...
End Function
Function OpenKey(SubKey As String)

Call ParseKey(SubKey, MainKeyHandle)

If MainKeyHandle Then
   rtn = RegOpenKeyEx(MainKeyHandle, SubKey, 0, KEY_READ, hKey) 'Opens the Key
   'If rtn = ERROR_SUCCESS Then 'if the key was opened then
      'rtn = RegCloseKey(hKey)  'close the key
   'End If
End If
